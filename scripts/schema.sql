...
-- ── Admin Note Replies ───────────────────────────────────
-- Private 1:1 replies from a note recipient back to the original note author.

create table admin_note_replies (
  id bigint generated always as identity primary key,
  note_id bigint references admin_notes(id) on delete cascade not null,
...
create index admin_note_replies_note_id_idx on admin_note_replies (note_id);
create index admin_note_replies_replied_by_idx on admin_note_replies (replied_by);
