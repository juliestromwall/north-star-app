...
| PdfOverlaySigner | src/components/esign/PdfOverlaySigner.jsx | Renders an `pdf-overlay` template (e.g. Kaiser PHI Release): pre-fills + flattens AcroForm fields via pdf-lib, then renders the resulting PDF with pdfjs as a background image with absolutely-positioned signature widget pulled from the named `gcSignature` widget rect. Mobile-friendly dedicated "Your signature" card below the PDF synced to the inline coord widget. `?calibrate=1` adds a 50pt grid + click-to-log coords. |
| pdfOverlay (lib) | src/lib/pdfOverlay.js | Core PDF baking: AcroForm fill (preferred) + coordinate fallback. `bakePdfOverlay()` flattens values + signature into a copy of the original PDF. `appendAuditTrailPage()` adds a native ESIGN/UETA "Electronic Signature Certificate" page (signer, email, timestamp, signature type, user agent) to the baked blob. DOB → MM/DD/YYYY, phone → "AAA    NNN-NNNN" Kaiser layout. |
...
| /api/notify-ip-app-released | functions/api/notify-ip-app-released.js | Admin releases application to IP → emails IP1+IP2 via Resend, "Welcome to North Star Surrogacy" copy with Log In & Complete button |
...
|-------|-------|-----------------|
| Pre-Qualification | #D4A853 (pink) | New, 1st/2nd/3rd Reach Out, Screening Call Scheduled/Complete, Pending Profile Completion, Profile Complete, Zoom Call Scheduled |
...
| Journey Closed | #1A3638 (indigo) | Closed — Complete/Withdrawn/Disqualified |
...
| 2026-05-01 | **Completed Journeys workflow**: "Mark Journey Complete" action on JourneyDetailPage gated on Escrow Closed checklist step. Stamps `journey_data._completedAt` + sets status `"Closed — Complete"` + archives. Hides both parties from default surrogate/IP lists (still searchable by name with green "Completed Journey" badge). New "Start Sibling Journey for <IP>" button on JourneyDetailPage IP card after completion (and on IPDetailPage hero) — wraps existing `startNewCaseFromJourney()` to spawn a fresh IP intake. New `isJourneyCompleted` + `fetchCompletedJourneys` helpers in `matching.js`. New `isJourneyEscrowClosed` helper in `checklistStore.js` (handles duplicate-label steps). Master Admins now default to "All Cases" view (was: only SuperAdmin + hardcoded email allowlist). Stage chip counts + "X of Y shown" header now exclude completed-journey cases to match what's rendered. |
| 2026-04-30 | **Venmo last-4 phone requirement** on GC Application Payment Preference (4-digit numeric, required). **IP intake cleanup**: removed egg/sperm donor questions (already on profile + clinic), moved criminal-history Y/N + describe to new "Background" section under Application Contact Info (with intake prefill). **Notify-profile-submitted email** link now points to `/surrogates/:caseId` instead of generic `/case-updates`. **Match Sheet**: IVF Physician shows "Dr." prefix, new IVF Clinic field, EditableSelect dropdowns portal to `document.body` so they aren't clipped by `InfoGrid` `overflow:hidden`. **Documents tab**: top-level "Upload File" button + Journey folder option (uploads doc to both files in matched view, default in matched view is now Journey). |
