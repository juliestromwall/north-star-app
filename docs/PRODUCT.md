...
| Master Admin default = All Cases | Both `/surrogates` and `/intended-parents` list pages default Master Admins (and Super Admins) to the "All Cases" view. Other admins default to "My Cases". Old behavior was Super Admins only + a hardcoded email allowlist (`julie@abcsurrogacy.com`, `nicole@abcsurrogacy.com`); the allowlist is kept as a fallback for non-master admins who need the wider view. |
